      ...
      <div class="row">
        ...
        <div class="col-xs-12">
          ...
          <div class="box box-primary">
            ...
            <!-- form start -->
            <form class="form-horizontal" id="frmData" method="post" action="{{ url('/konsumen-barang') }}/{{ $konsumen_barang->id }}" autocomplete="off">
              ...
              <div class="box-body">
                  ...
                  <div class="col-sm-10">
                    <select name="konsumen_id" id="konsumen_id" class="form-control selectpicker" title="-- Pilih konsumen --">
                      @foreach($konsumen as $item)
                        <option value="{{ $item->id }}" {{ $item->id == (old('konsumen_id') !== null ? old('konsumen_id') : $konsumen_barang->konsumen_id) ? 'selected' : '' }} >{{ $item->nama }}</option>
                      @endforeach
                    </select>
                  </div>
                </div>
                  ...
                  <div class="col-sm-10">
                    <select name="barang_id" id="barang_id" class="form-control selectpicker" title="-- Pilih barang --">
                      @foreach($barang as $item)
                        <option value="{{ $item->id }}" {{ $item->id == (old('barang_id') !== null ? old('barang_id') : $konsumen_barang->barang_id) ? 'selected' : '' }} >{{ $item->nama }}</option>
                      @endforeach
                    </select>
                  </div>
                </div>
                <div class="form-group">
                  ...
                  <div class="col-sm-10">
                    <input type="text" class="form-control" name="harga" id="harga" placeholder="Biaya" value="{{ old('harga') !== null ? str_replace(',', '', old('harga')) : $konsumen_barang->harga }}">
                  </div>
                </div>
